...
def process_export(export_id, user_pk=None):
    ...
    owner = export.owner
    oldest_exports_qs = Export.objects.filter(owner=owner).order_by("-created_on").all()
    ...
    if len(oldest_exports) > settings.EXPORTS_TO_KEEP:
        ...
        Export.objects.filter(owner=owner).filter(
            created_on__lt=oldest_export.created_on
        ).delete()
