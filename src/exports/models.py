...
class Export(models.Model):
    ...
    owner = models.ForeignKey(
        "accounts.User", on_delete=models.PROTECT, verbose_name=_("Owner")
    )
    category = models.ForeignKey(
        "categories.Category", on_delete=models.PROTECT, verbose_name=_("Category")
    )
    ...
    status = models.CharField(
        _("Status"), max_length=30, choices=STATUSES, default=STATUSES.new
    )
    format = models.CharField(
        _("Format"), max_length=5, choices=FORMATS, default=FORMATS.csv
    )
    ...
    def get_filename(self):
        return "export_{time:%Y%m%d}_{uid}.{exten}".format(
            time=self.created_on, uid=self.id, exten=self.format
        )

    def get_url(self):
        return os.path.join(settings.EXPORTS_URL.lstrip("/"), self.get_filename())

    def get_filedir(self):
        return os.path.join(settings.PRIVATE_ROOT, settings.EXPORTS_SUBDIR)

    def get_filepath(self):
        return os.path.join(self.get_filedir(), self.get_filename())

    def start_export(self, user_pk=None):
