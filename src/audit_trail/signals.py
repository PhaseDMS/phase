...
activity_log = Signal(
    providing_args=["actor", "verb", "action_object", "action_type", "target"]
)
...
def activity_handler(verb, action_object=None, target=None, **kwargs):
    ...
    if isinstance(actor, User):
        ...
        raise ValueError(
            "Actor must be a user instance or belong to " "Activity NON_DB_USERS"
        )
    ...
    activity.action_object_str = kwargs.get("action_object_str", None) or get_repr(
        action_object
    )
    activity.save()
