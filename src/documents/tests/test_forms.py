...
from default_documents.models import DemoMetadataRevision, ContractorDeliverable
...
class DocumentCreateTest(TestCase):
    ...
    def test_creation_errors_with_files(self):
        ...
        c = self.client
        with open(
            join(self.sample_path, "sample_doc_native.docx"), mode="rb"
        ) as native_file:
            with open(
                join(self.sample_path, "sample_doc_pdf.pdf"), mode="rb"
            ) as pdf_file:
                r = c.post(
                    self.create_url,
                    {
                ...
                self.assertEqual(r.status_code, 200)
                self.assertEqual(
                    r.context["revision_form"].errors,
                    {
    ...
    def test_creation_success(self):
        ...
        if r.status_code == 302:
            self.assertEqual(
                original_number_of_document + 1, Document.objects.all().count()
            )
        else:
            # Debug purpose
    ...
    def test_creation_redirect(self):
        ...
        self.assertEqual(r.redirect_chain, [(self.category.get_absolute_url(), 302)])
...
class DocumentReviseTest(TestCase):
    ...
    def test_new_revision_id_is_bumped(self):
        ...
        revision = DemoMetadataRevision.objects.filter(
    ...
    def test_new_revision_files(self):
        ...
        with open(
            join(sample_path, "sample_doc_native.docx"), mode="rb"
        ) as native_file:
        ...
        self.assertTrue(revision.native_file.name.startswith(fn_begin))
        self.assertTrue(revision.pdf_file.name.startswith(fn_begin))
        self.assertTrue(revision.native_file.name.endswith(".docx"))
        self.assertTrue(revision.pdf_file.name.endswith(".pdf"))
