...
from distriblists.utils import (
    import_lists,
    export_lists,
    export_review_members,
    import_review_members,
)
...
class DistributionListsImportTests(TestCase):
    ...
    def test_successful_import(self):
        ...
        xls_file = os.path.join(
            os.path.dirname(__file__), "fixtures", "valid_distrib_list.xlsx"
        )
    ...
    def test_invalid_data_no_leader(self):
        ...
        xls_file = os.path.join(
            os.path.dirname(__file__), "fixtures", "distrib_list_missing_leader.xlsx"
        )
        import_lists(xls_file, self.category)
    ...
    def test_invalid_data_unknown_user(self):
        ...
        xls_file = os.path.join(
            os.path.dirname(__file__), "fixtures", "distrib_list_unknown_user.xlsx"
        )
        import_lists(xls_file, self.category)
    ...
    def test_invalid_data_invalid_category(self):
        ...
        xls_file = os.path.join(
            os.path.dirname(__file__), "fixtures", "distrib_list_wrong_category.xlsx"
        )
        import_lists(xls_file, self.category)
...
class ReviewMembersExportTests(ContractorDeliverableTestCase):
    def setUp(self):
        ...
        self.docs = [
            self.create_doc(revision={"leader": self.users[0]}),
            self.create_doc(
                revision={"leader": self.users[1], "approver": self.users[0]}
            ),
            self.create_doc(
                revision={
                    "leader": self.users[0],
...
class ReviewMembersImportTests(ContractorDeliverableTestCase):
    ...
    def test_successful_import(self):
        ...
        xls_file = os.path.join(
            os.path.dirname(__file__), "fixtures", "valid_review_members.xlsx"
        )
