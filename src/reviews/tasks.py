...
@app.task
def do_batch_import(user_id, category_id, contenttype_id, document_ids, remark=None):

    # Fetch document list
    contenttype = ContentType.objects.get_for_id(contenttype_id)
    document_class = contenttype.model_class()
    docs = (
        document_class.objects.select_related()
    ...
    if len(ok) > 0:
        ...
        ok_list = "</li><li>".join(
            '<a href="%s">%s</a>' % (doc.get_absolute_url(), doc) for doc in ok
        )
        notify(user_id, "{} <ul><li>{}</li></ul>".format(ok_message, ok_list))

    if len(nok) > 0:
        nok_message = ugettext(
            "We failed to start the review for the following documents:"
        )
        nok_list = "</li><li>".join(
            '<a href="%s">%s</a>' % (doc.get_absolute_url(), doc) for doc in nok
        )
        notify(user_id, "{} <ul><li>{}</li></ul>".format(nok_message, nok_list))
...
def batch_close_reviews(user_id, review_ids):
    ...
    """
    logger.info("Closing several reviews at once: {}".format(", ".join(review_ids)))

    ok = []
    nok = []
    nb_reviews = len(review_ids)
    reviews = (
        Review.objects.filter(reviewer_id=user_id)
    ...
    if len(nok) > 0:
        nok_message = ugettext(
            "We failed to close the review for the following documents:"
        )
        nok_list = "</li><li>".join(
            '<a href="%s">%s</a>' % (doc.get_absolute_url(), doc) for doc in nok
        )
...
def batch_cancel_reviews(user_id, category_id, contenttype_id, document_ids):
    ...
    if len(nok) > 0:
        nok_message = ugettext(
            "We failed to cancel the review for the following documents:"
        )
        nok_list = "</li><li>".join(
            '<a href="%s">%s</a>' % (doc.get_absolute_url(), doc) for doc in nok
        )
