...
class Review(models.Model):
    ...
    start_date = models.DateField(_("Review start date"), null=True, blank=True)
    due_date = models.DateField(_("Review due date"), null=True, blank=True)
    docclass = models.IntegerField(verbose_name="Class", default=1, choices=CLASSES)
    status = models.CharField(
        _("Status"), max_length=30, choices=STATUSES, default=STATUSES.pending
    )
    revision_status = models.CharField(
        _("Revision status"), max_length=30, null=True, blank=True
    )
    ...
    comments = PrivateFileField(
        _("Comments"), null=True, blank=True, upload_to=review_comments_file_path
    )
...
class ReviewMixin(models.Model):
    ...
    reviewers_step_closed = models.DateField(
        _("Reviewers step closed"), null=True, blank=True
    )
    leader_step_closed = models.DateField(
        _("Leader step closed"), null=True, blank=True
    )
    review_end_date = models.DateField(_("Review end date"), null=True, blank=True)
    reviewers = models.ManyToManyField(User, verbose_name=_("Reviewers"), blank=True)
    ...
    def can_be_reviewed(self):
        ...
        """
        return all((self.received_date, self.leader_id, not self.review_start_date))
    ...
    def start_review(self, at_date=None, due_date=None):
        ...
        self.review_due_date = due_date or self.received_date + datetime.timedelta(
            days=duration
        )
    ...
    def cancel_review(self):
        ...
        """
        Review.objects.filter(document=self.document).filter(
            revision=self.revision
        ).delete()
    ...
    def end_reviewers_step(self, at_date=None, save=True):
        ...
        Review.objects.filter(document=self.document).filter(
            revision=self.revision
        ).filter(role=Review.ROLES.reviewer).filter(closed_on=None).update(
            closed_on=end_date, status="not_reviewed"
        )

        Review.objects.filter(document=self.document).filter(
            revision=self.revision
        ).filter(role=Review.ROLES.leader).update(status="progress")
    ...
    def end_leader_step(self, at_date=None, save=True):
        ...
        Review.objects.filter(document=self.document).filter(
            revision=self.revision
        ).filter(role=Review.ROLES.leader).filter(closed_on=None).update(
            closed_on=end_date, status="not_reviewed"
        )

        Review.objects.filter(document=self.document).filter(
            revision=self.revision
        ).filter(role=Review.ROLES.approver).update(status="progress")
    ...
    def send_back_to_leader_step(self, save=True):
        ...
        Review.objects.filter(document=self.document).filter(
            revision=self.revision
        ).filter(role=Review.ROLES.leader).update(closed_on=None, status="progress")
    ...
    def end_review(self, at_date=None, save=True):
        ...
        Review.objects.filter(document=self.document).filter(
            revision=self.revision
        ).filter(role=Review.ROLES.approver).filter(closed_on=None).update(
            closed_on=end_date, status="not_reviewed"
        )
    ...
    def get_reviews(self):
        ...
        if not hasattr(self, "_reviews"):
            qs = (
                Review.objects.filter(document=self.document)
    ...
    def get_review_duration(self):
        ...
        if review_duration is None:
            raise ImproperlyConfigured(
                ...
                "override {0}.get_review_duration().".format(self.__class__.__name__)
            )
    ...
    def post_trs_import(self, trs_revision):
        ...
        if trs_revision.leader_comment_date:
            self.end_leader_step(at_date=trs_revision.leader_comment_date, save=False)

        # Is there an approver and did he submit a comment?
        if self.approver_id and trs_revision.approver_comment_date:
            self.end_review(at_date=trs_revision.approver_comment_date, save=False)
    ...
    def detail_view_context(self, request):
        ...
        context.update(
            {"user_review": user_review, "review_closed_on": review_closed_on}
        )
        return context
            ...
            if can_start_stop_review:
                actions.insert(
                    ...
                    MenuItem(
                        ...
                        _("Cancel review"),
                        reverse(
                            "document_cancel_review", args=[self.document.document_key]
                        ),
            ...
            if review_closed_on:
                actions.insert(
                    ...
                    MenuItem(
                        ...
                        reverse("review_document", args=[self.document.document_key]),
                        method="GET",
                    ),
                )
