...
from reviews.tasks import do_batch_import, batch_close_reviews, batch_cancel_reviews
...
class StartReview(PermissionRequiredMixin, DocumentListMixin, SingleObjectMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if revision.can_be_reviewed:
            ...
            activity_log.send(
                verb=Activity.VERB_STARTED_REVIEW,
...
class CancelReview(PermissionRequiredMixin, SingleObjectMixin, View):
    ...
    def get_object(self, queryset=None):
        ...
        qs = Document.objects.filter(category__users=self.request.user)
    ...
    def post(self, request, *args, **kwargs):
        ...
        if revision.is_under_review():
            ...
            notify(request.user, _(message_text) % message_data)
            activity_log.send(
                verb=Activity.VERB_CANCELLED_REVIEW,
                target=revision,
                sender=None,
                actor=self.request.user,
            )
...
class BatchCancelReviews(PermissionRequiredMixin, BaseDocumentBatchActionView):
    ...
    def start_job(self, contenttype, document_ids):
        job = batch_cancel_reviews.delay(
            self.request.user.id, self.category.id, contenttype.id, document_ids
        )
        return job
...
class BaseReviewDocumentList(LoginRequiredMixin, ListView):
    ...
    def get_search_form(self, reviews):
        form = ReviewSearchForm(
            self.request.GET or None, user=self.request.user, reviews=reviews
        )
        return form
...
class PrioritiesDocumentList(BaseReviewDocumentList):
    ...
    def step_filter(self, qs):
        ...
        qs = qs.filter(role_q).filter(due_date__lte=delta).filter(docclass__lte=2)
        return qs
...
class ReviewFormView(LoginRequiredMixin, UpdateView):
    ...
    def get_object(self, queryset=None):
        document_key = self.kwargs.get("document_key")
        qs = Document.objects.filter(category__users=self.request.user).select_related()
        document = get_object_or_404(qs, document_key=document_key)
    ...
    def get_context_data(self, **kwargs):
        ...
        close_reviewers_button = self.revision.is_at_review_step("reviewer") and (
            is_leader or is_approver
        )
        close_leader_button = self.revision.is_at_review_step("leader") and is_approver
        back_to_leader_button = (
            self.revision.is_at_review_step("approver") and is_approver
        )
    ...
    def form_valid(self, form):
        ...
        if "close_reviewers_step" in self.request.POST and user in (
            self.revision.leader,
            self.revision.approver,
        ):
        ...
        if (
            "back_to_leader_step" in self.request.POST
            and user == self.revision.approver
        ):
        ...
        if verb:
            activity_log.send(
                verb=verb, target=self.revision, sender=None, actor=self.request.user
            )
...
class CommentsDownload(LoginRequiredMixin, DetailView):
    ...
    def get_object(self, queryset=None):
        ...
        qs = (
            Review.objects.filter(document__document_key=key)
