...
from transmittals.fields import (
    TransmittalFileField,
    OgtFileField,
    ClientCommentsFileField,
)
...
class Transmittal(Metadata):
    ...
    transmittal_key = models.CharField(_("Transmittal key"), max_length=250)

    # General informations
    transmittal_date = models.DateField(_("Transmittal date"), null=True, blank=True)
    ack_of_receipt_date = models.DateField(
        _("Acknowledgment of receipt date"), null=True, blank=True
    )
    ...
    originator = ConfigurableChoiceField(
        _("Originator"), default="CTR", max_length=3, list_index="ORIGINATORS"
    )
    ...
    sequential_number = models.PositiveIntegerField(
        _("sequential number"), null=True, blank=True
    )
    document_type = ConfigurableChoiceField(
        _("Document Type"), default="PID", max_length=3, list_index="DOCUMENT_TYPES"
    )
    status = models.CharField(
        max_length=20, choices=STATUSES, default=STATUSES.tobechecked
    )

    # Related documents
    related_documents = models.ManyToManyField(
        "documents.Document", related_name="transmittals_related_set", blank=True
    )
    ...
    def reject(self):
        ...
        if self.status != "tobechecked":
            error_msg = (
                "The transmittal {} cannot be rejected "
                "it it's current status ({})".format(self.document_key, self.status)
            )
        ...
        if os.path.exists(self.full_tobechecked_name):
            try:
                ...
                logger.error("Cannot reject transmittal {} ({})".format(self, e))
                raise e
        else:
        ...
        new_key = "{}-{}".format(self.document_key, uuid.uuid4())
    ...
    def accept(self):
        ...
        if self.status != "tobechecked":
            error_msg = (
                "The transmittal {} cannot be accepted "
...
class TransmittalRevision(MetadataRevision):
    ...
    trs_status = ConfigurableChoiceField(
        _("Status"), max_length=20, default="opened", list_index="STATUS_TRANSMITTALS"
    )
...
class TrsRevision(models.Model):
    ...
    transmittal = models.ForeignKey(
        Transmittal, on_delete=models.PROTECT, verbose_name=_("Transmittal")
    )
    ...
    document_key = models.SlugField(_("Document number"), max_length=250)
    category = models.ForeignKey("categories.Category", on_delete=models.PROTECT)
    title = models.TextField(verbose_name=_("Title"))
    revision = models.PositiveIntegerField(verbose_name=_("Revision"), default=0)
    ...
    unit = ConfigurableChoiceField(
        verbose_name=_("Unit"), default="000", max_length=3, list_index="UNITS"
    )
    ...
    system = ConfigurableChoiceField(
        _("System"), list_index="SYSTEMS", null=True, blank=True
    )
    wbs = ConfigurableChoiceField(
        _("Wbs"), max_length=20, list_index="WBS", null=True, blank=True
    )
    weight = models.IntegerField(_("Weight"), null=True, blank=True)
    docclass = models.IntegerField(verbose_name=_("Class"), default=1, choices=CLASSES)
    ...
    return_code = models.PositiveIntegerField(_("Return code"), null=True, blank=True)
    review_start_date = models.DateField(_("Review start date"), null=True, blank=True)
    review_due_date = models.DateField(_("Review due date"), null=True, blank=True)
    review_leader = models.CharField(
        _("Review leader"), max_length=150, null=True, blank=True
    )
    leader_comment_date = models.DateField(
        _("Leader comment date"), null=True, blank=True
    )
    review_approver = models.CharField(
        _("Review approver"), max_length=150, null=True, blank=True
    )
    approver_comment_date = models.DateField(
        _("Approver comment date"), null=True, blank=True
    )
    review_trs = models.CharField(
        verbose_name=_("Review transmittal name"), max_length=255, null=True, blank=True
    )
    ...
    outgoing_trs_sent_date = models.DateField(
        verbose_name=_("Outgoing transmittal sent date"), null=True, blank=True
    )
    doc_category = models.CharField(max_length=50, verbose_name=_("Doc category"))
    pdf_file = TransmittalFileField(verbose_name=_("Pdf file"))
    native_file = TransmittalFileField(
        verbose_name=_("Native file"), null=True, blank=True, max_length=255
    )
    ...
    def get_document_fields(self):
        ...
        files_dict = {"native_file": self.native_file, "pdf_file": self.pdf_file}
...
class OutgoingTransmittal(Metadata):
    ...
    PURPOSE_OF_ISSUE_CHOICES = Choices(
        ("FR", _("For review")), ("FI", _("For information"))
    )
    ...
    revisions_category = models.ForeignKey(
        "categories.Category", verbose_name=_("From category"), on_delete=models.PROTECT
    )
    ...
    contract_number = models.CharField(verbose_name="Contract Number", max_length=50)
    originator = models.CharField(_("Originator"), max_length=3)
    recipient = models.ForeignKey(
        "accounts.Entity", on_delete=models.PROTECT, verbose_name=_("Recipient")
    )
    sequential_number = models.PositiveIntegerField(
        _("sequential number"), null=True, blank=True
    )
    ...
    archived_pdf = OgtFileField(verbose_name=_("Archived PDF"), null=True, blank=True)
    ...
    external_review_due_date = models.DateField(
        _("External due date"), null=True, blank=True
    )
    ...
    def get_batch_actions(cls, category, user):
        actions = super(OutgoingTransmittal, cls).get_batch_actions(category, user)
    ...
    def get_batch_actions_modals(cls):
        ...
        return [
            "transmittals/document_list_download_modal.html",
...
class OutgoingTransmittalRevision(MetadataRevisionBase):
    ...
    error_notified = models.BooleanField(
        _("Was error already notified?"), default=False
    )
    ...
    def get_actions(self, metadata, user):
        actions = super(OutgoingTransmittalRevision, self).get_actions(metadata, user)
...
class ExportedRevision(models.Model):
    ...
    comments = ProtectedFileField(_("Comments"), null=True, blank=True)
...
class TransmittableMixin(ReviewMixin):
    ...
    internal_review = models.BooleanField(
        ...
        choices=Choices((False, "No"), (True, "Yes")),
        default=False,
    )
    client_comments = ClientCommentsFileField(
        verbose_name=_("Client Comments"), null=True, blank=True
    )
    client_comments_detail = models.TextField(
        verbose_name=_("Client comments detais"), blank=True
    )
    ...
    def get_initial_empty(self):
        ...
        return empty_fields + ("file_transmitted",)
