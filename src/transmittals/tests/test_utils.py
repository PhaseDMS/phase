...
from transmittals.factories import (
    OutgoingTransmittalFactory,
    OutgoingTransmittalRevisionFactory,
)
...
class TransmittalCreationTests(ContractorDeliverableTestCase):
    ...
    def setUp(self):
        ...
        self.dst_category = CategoryFactory(category_template__metadata_model=Model)
        self.user1 = UserFactory(
            email="user1@phase.fr", password="pass", category=self.category
        )
        ...
        self.linked_contract = ContractFactory.create(categories=[self.category])
