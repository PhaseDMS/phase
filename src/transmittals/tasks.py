...
from transmittals.utils import (
    create_transmittal,
    send_transmittal_creation_notifications,
)
...
def do_notify_transmittal_recipients(metadata_id, revision_id):
    ...
    logger.info(
        "Notifying transmittal recipients. Meta={} Rev={}".format(
            metadata_id, revision_id
        )
    )

    transmittal = (
        OutgoingTransmittal.objects.select_related("document", "recipient")
...
def do_create_transmittal(
    ...
    documents = Document.objects.select_related().filter(id__in=document_ids)
    revisions = []
    ...
    try:
        for recipient in recipients:
            ...
            activity_log.send(
                verb=Activity.VERB_CREATED, action_object=doc, sender=None, actor=user
            )

    except TransmittalError as e:
        msg = """We failed to create a transmittal for the
                 following reason: "{}".""".format(
            e
        )
        notify(user_id, msg)
...
def process_transmittal(transmittal_id):
    ...
    revisions = (
        TrsRevision.objects.filter(transmittal=transmittal)
        .order_by("revision")
        .select_related()
    )
    ...
    try:
        ...
        if os.path.exists(transmittal.full_tobechecked_name):
            try:
                os.rename(
                    transmittal.full_tobechecked_name, transmittal.full_accepted_name
                )
            except OSError as e:
                logger.error("Cannot move transmittal {} ({})".format(transmittal, e))
        else:
        ...
        success_msg = "Transmittal {} successfully processed".format(transmittal)
