...
class UserManager(BaseUserManager):
    ...
    def create_superuser(
        self, email, name, username=None, password=None, **extra_fields
    ):
...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    username = models.SlugField(
        _("Username"), unique=True, db_index=True, max_length=64
    )
    ...
    is_external = models.BooleanField(
        ...
        default=False,
        help_text=_(
            "Used to tell apart regular users and externals ones" " (contractors)."
        ),
    )
    date_joined = models.DateTimeField(_("date joined"), default=timezone.now)

    send_closed_reviews_mails = models.BooleanField(
        _("Send mails when reviews are closed"), default=True
    )
    send_pending_reviews_mails = models.BooleanField(
        _("Send pending reviews reminders"), default=True
    )
    send_trs_reminders_mails = models.BooleanField(
        _(
            "Send reminders for outgoing transmittals with missing "
    ...
    def send_account_activation_email(self, token, site=None):
        ...
        subject = render_to_string("registration/activation_email_subject.txt", ctx)
        # Remove new lines
...
class Entity(models.Model):
    ...
    type = models.CharField(
        _("Type"), max_length=80, choices=TYPES, default=TYPES.contractor
    )
